...
# ---- 4. sampling-depth abundance table (placeholder, edit later) -----------
# 9 Gourgouthakas sampling depths (metres). Random counts are generated once
# into results/ if the file is absent; replace with real numbers and re-run.
...
if (!file.exists(depth_tsv)) {
  ...
  if (length(missing) > 0) {
    ...
    message("added ", length(missing), " missing taxa to ", depth_tsv,
